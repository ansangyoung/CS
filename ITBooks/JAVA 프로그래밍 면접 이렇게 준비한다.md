...
숫자(number)타입만 사용할 수 있었는데, 자바 5에서는 열거형(enumerate) 타입을, 자바 7에서는 문자열 객체(String Object)를 사용할 수 있도록 발전했다.  
  
  
  
## 04 기본 알고리즘 작성하기  
### 빅 오 표현법  
빅오(Big O) 표현법은 알고리즘의 성능이나 복잡도를 설명하는 데 일반적으로 사용하는 방법이다.  
입력 값이 바뀌었을 때 알고리즘의 성능이 어떻게 바뀌는지를 알려준다.  
대부분의 경우 알고리즘의 원하는 정확한 의도를 표현한다고 하더라도, O(n^2)은 그다지 효율적인 방법이 아니다.  
  
알고리즘을 작성할 때는 시간/공간 복잡성을 함께 고려해야 한다.  
...
LinkedList는 자료의 주소 값으로 서로 연결되어 있는 구조를 가지고 있다.  
LinkedList는 뒤로 밀거나 채우는 작업 없이 삽입, 삭제가 용이하지만, 순차접근만 가능한 단점이 있다.  
...
보통 객체를 미리 생성해두고 가져오는 단순하고 안전한 방법을 사용한다.  
최초 한번의 new 연산자를 통해서 고정된 메모리 영역을 사용하기 때문에 추후 해당 객체에 접근할 때 메모리 낭비를 방지할 수 있으며, 속도 측면에서도 이점이 있다.  
동시성에 문제가 있을 수 있지만, 다른 클래스 간에 데이터 공유가 쉽다.  
대표적으로 log4j를 사용할 때 사용한다.  
...
private 멤버 변수는 오직 해당 클래스에서만 사용할 수 있다. 하위 클래스에서도 사용할 수 없다.  
private 변수는 오직 해당 타입만 필요하다고 간주한다.  
  
일반적인 오해는 private으로 선언한 변수는 해당 인스턴스에만 접근할 수 있다고 생각하는 것이다.  
실제로 같은 타입의 다른 모든 인스턴스가 private 멤버 변수에 접근할 수 있다.  
즉, 같은 타입이지만 다른 인스턴스에서 private 멤버 변수에 접근할 수 있다.  
